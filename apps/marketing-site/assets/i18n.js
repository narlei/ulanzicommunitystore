...
(function (global) {
  ...
  var strings = {
    en: {
      ...
      hero_lead: 'Install and update community plugins in one click — or publish your own without forking anything.',
      ...
      window_nav_store: 'Store',
      window_nav_installed: 'Installed',
      window_nav_updates: 'Updates',
      window_nav_send: 'Send plugin',
      window_nav_settings: 'Settings',
      window_star: 'Star on GitHub',
      window_badge_updated: 'Updated',
      window_badge_new: 'New',
      ...
      catalog_share: 'Share',
      catalog_share_copied: 'Link copied!',
      catalog_open_app: 'Open in app',
      catalog_source: 'Source',
      catalog_report_problem: 'Report a Problem',
      catalog_more_actions: 'More actions',
      catalog_about: 'About',
      ...
      // What's new
      updates_meta_title: "What's new — Ulanzi Community Store",
      updates_meta_description: 'New plugins and updates in the Ulanzi Community Store, week by week. Share any edition — the link keeps showing the same period.',
      updates_eyebrow: "What's new",
      updates_title: 'New plugins & updates',
      updates_lead: 'Everything that landed in the community registry during the selected period. Share the link — it always shows this exact window.',
      updates_prev: 'Previous period',
      updates_next: 'Next period',
      updates_range: '%s → %s',
      updates_summary: '%s new · %s updated',
      updates_author_count: '%s in this period',
      updates_section_new: 'New in the store',
      updates_section_updated: 'Updated',
      updates_added_on: 'Added %s',
      updates_released_on: 'Released %s',
      updates_to_version: 'Updated to v%s',
      updates_more_releases: '+%s earlier in this period',
      updates_share: 'Copy link',
      updates_share_copied: 'Link copied',
      updates_loading: 'Loading the catalog…',
      updates_error: "Couldn't load the catalog. Try again in a moment.",
      updates_empty_title: 'Nothing shipped in this period.',
      updates_empty_body: 'Try the previous week, or browse the full catalog.',
      updates_browse_all: 'Browse all plugins',
      ...
      starter_2_title: 'Scaffold or adapt',
      starter_2_body: 'In an empty folder (new plugin) or inside an existing plugin repo (adapt mode), run the command below. It generates a complete structure: <code>Makefile</code>, GitHub Actions release pipeline, optional <code>store.json</code>, and an AI skill for vibe-coding.',
      starter_store_help: 'Already have a working plugin and only need cover, screenshots and tags?',
      ...
      publish_1_title: 'Scaffold or adapt with the starter',
      publish_1_body: 'Use <a href="#starter-kit">ulanzi-plugin-starter</a> to create a new plugin or adapt an existing repo — Makefile, release workflow and <code>store.json</code> included. The <a href="https://github.com/UlanziTechnology/UlanziDeckPlugin-SDK" target="_blank" rel="noopener noreferrer">official Ulanzi SDK</a> remains the reference for the plugin API.',
      publish_2_title: 'Tag a GitHub Release',
      publish_2_body: 'Push a version tag (<code>git tag v1.0.0 &amp;&amp; git push origin v1.0.0</code>). The starter workflow builds and attaches <code>*.ulanziPlugin.zip</code>. Every new release becomes an update in the store.',
      publish_3_title: 'Submit your repository',
      publish_3_body: 'Paste your repo URL below — or in the app\'s <strong>Send plugin</strong> tab. We validate it and open the submission issue for you; a bot takes it from there.',
      ...
      publish_pr: 'Submit the plugin on GitHub',
      publish_copy_json: 'Copy JSON',
      publish_hint: 'No fork needed: the button opens a pre-filled submission issue. A bot validates your repo, opens the registry Pull Request, and once it is merged your plugin goes live automatically.',
      publish_doc: 'Want a cover image, screenshots and tags? Run <code>npx ulanzi-plugin-starter@latest store</code> in your repo — or see the <a href="https://github.com/narlei/ulanzicommunitystore/tree/main/plugin-starter" target="_blank" rel="noopener noreferrer">starter docs</a>.',
      publish_store_nudge: 'Optional: generate <code>store.json</code> for cover, screenshots and tags:',
      publish_fix_title: 'How to fix',
      publish_fix_repo_title: 'Invalid repository URL',
      publish_fix_repo: 'Use a public GitHub URL like <code>https://github.com/you/your-plugin</code> (or just <code>you/your-plugin</code>).',
      publish_fix_release_title: 'No release yet',
      publish_fix_release: 'If you used the starter, push a version tag — the workflow attaches the zip automatically:',
      publish_fix_asset_title: 'Release has no plugin zip',
      publish_fix_asset: 'The latest release needs a <code>*.ulanziPlugin.zip</code> asset. With the starter workflow, ensure the plugin folder is at the repo root, then tag again:',
      publish_fix_manifest_title: 'Plugin layout issue',
      publish_fix_manifest: 'Expect <code>com.&lt;you&gt;.&lt;plugin&gt;.ulanziPlugin/manifest.json</code> at the repo root. Run <strong>init</strong> inside your repo to adapt the layout without rewriting your code:',
      publish_fix_store_title: 'Make your listing shine',
      publish_fix_store: 'Optional but recommended — generate <code>store.json</code> for cover, screenshots, tags and a long description:',
      publish_fix_store_invalid: 'Your <code>store.json</code> is not valid JSON. Regenerate it or fix the syntax:',
      ...
      publish_release_fail: 'No GitHub Release found. Tag a version first — the starter ships a release workflow.',
      publish_asset_ok: 'Release asset: %s',
      publish_asset_fail: 'Latest release has no *.ulanziPlugin.zip. With the starter, push a version tag after the workflow is in place.',
      ...
      publish_store_warn: 'No store.json (optional). Run the store command for cover, screenshots and tags.',
    ...
    pt: {
      ...
      hero_lead: 'Instale e atualize plugins da comunidade com um clique — ou publique o seu sem precisar forkar nada.',
      hero_download: 'Baixar grátis',
      hero_publish: 'Publicar um plugin',
      hero_no_account: 'Sem conta necessária',
      window_search: 'Buscar…',
      window_installed: 'Instalado',
      window_installing: 'Instalando…',
      window_install: 'Instalar',
      window_nav_store: 'Loja',
      window_nav_installed: 'Instalados',
      window_nav_updates: 'Atualizações',
      window_nav_send: 'Enviar plugin',
      window_nav_settings: 'Ajustes',
      window_star: 'Estrela no GitHub',
      window_badge_updated: 'Atualizado',
      window_badge_new: 'Novo',
      ...
      catalog_share_copied: 'Link copiado!',
      catalog_open_app: 'Abrir no app',
      catalog_source: 'Código',
      catalog_report_problem: 'Reportar um problema',
      catalog_more_actions: 'Mais ações',
      catalog_about: 'Sobre',
      ...
      updates_meta_title: 'Novidades — Ulanzi Community Store',
      updates_meta_description: 'Plugins novos e atualizações da Ulanzi Community Store, semana a semana. Compartilhe qualquer edição — o link continua mostrando o mesmo período.',
      updates_eyebrow: 'Novidades',
      updates_title: 'Plugins novos & atualizações',
      updates_lead: 'Tudo que entrou no registro da comunidade no período selecionado. Compartilhe o link — ele sempre mostra exatamente esta janela.',
      updates_prev: 'Período anterior',
      updates_next: 'Próximo período',
      updates_range: '%s → %s',
      updates_summary: '%s novos · %s atualizados',
      updates_author_count: '%s neste período',
      updates_section_new: 'Novos na store',
      updates_section_updated: 'Atualizados',
      updates_added_on: 'Publicado em %s',
      updates_released_on: 'Lançado em %s',
      updates_to_version: 'Atualizado para v%s',
      updates_more_releases: '+%s antes neste período',
      updates_share: 'Copiar link',
      updates_share_copied: 'Link copiado',
      updates_loading: 'Carregando o catálogo…',
      updates_error: 'Não foi possível carregar o catálogo. Tente de novo em instantes.',
      updates_empty_title: 'Nada foi publicado neste período.',
      updates_empty_body: 'Veja a semana anterior ou explore o catálogo completo.',
      updates_browse_all: 'Ver todos os plugins',
      ...
      starter_2_title: 'Crie ou adapte',
      starter_2_body: 'Em uma pasta vazia (plugin novo) ou dentro de um repo existente (modo adapt), rode o comando abaixo. Ele gera a estrutura completa: <code>Makefile</code>, pipeline de release no GitHub Actions, <code>store.json</code> opcional e skill de IA para vibe-coding.',
      starter_store_help: 'Já tem um plugin funcionando e só precisa de capa, capturas e tags?',
      ...
      publish_1_title: 'Crie ou adapte com o starter',
      publish_1_body: 'Use o <a href="#starter-kit">ulanzi-plugin-starter</a> para criar um plugin novo ou adaptar um repo existente — Makefile, workflow de release e <code>store.json</code> incluídos. O <a href="https://github.com/UlanziTechnology/UlanziDeckPlugin-SDK" target="_blank" rel="noopener noreferrer">SDK oficial da Ulanzi</a> continua sendo a referência da API do plugin.',
      publish_2_title: 'Crie um GitHub Release com tag',
      publish_2_body: 'Envie uma tag de versão (<code>git tag v1.0.0 &amp;&amp; git push origin v1.0.0</code>). O workflow do starter monta e anexa o <code>*.ulanziPlugin.zip</code>. Cada novo release vira uma atualização na loja.',
      publish_3_title: 'Envie seu repositório',
      publish_3_body: 'Cole a URL do repo abaixo — ou na aba <strong>Enviar plugin</strong> do app. Validamos e abrimos a issue de publicação para você; o resto o bot faz.',
      ...
      publish_pr: 'Publicar o plugin no GitHub',
      publish_copy_json: 'Copiar JSON',
      publish_hint: 'Sem fork: o botão abre uma issue de publicação já preenchida. Um bot valida seu repo, abre o Pull Request do registry e, quando ele for mesclado, seu plugin entra no ar automaticamente.',
      publish_doc: 'Quer capa, capturas e tags? Rode <code>npx ulanzi-plugin-starter@latest store</code> no repo — ou veja a <a href="https://github.com/narlei/ulanzicommunitystore/tree/main/plugin-starter" target="_blank" rel="noopener noreferrer">documentação do starter</a>.',
      publish_store_nudge: 'Opcional: gere um <code>store.json</code> para capa, capturas e tags:',
      publish_fix_title: 'Como corrigir',
      publish_fix_repo_title: 'URL do repositório inválida',
      publish_fix_repo: 'Use uma URL pública do GitHub como <code>https://github.com/voce/seu-plugin</code> (ou só <code>voce/seu-plugin</code>).',
      publish_fix_release_title: 'Ainda sem release',
      publish_fix_release: 'Se usou o starter, envie uma tag de versão — o workflow anexa o zip sozinho:',
      publish_fix_asset_title: 'Release sem zip do plugin',
      publish_fix_asset: 'A release mais recente precisa de um asset <code>*.ulanziPlugin.zip</code>. Com o workflow do starter, deixe a pasta do plugin na raiz do repo e crie a tag de novo:',
      publish_fix_manifest_title: 'Layout do plugin',
      publish_fix_manifest: 'Esperamos <code>com.&lt;voce&gt;.&lt;plugin&gt;.ulanziPlugin/manifest.json</code> na raiz do repo. Rode <strong>init</strong> dentro do repo para adaptar o layout sem reescrever o código:',
      publish_fix_store_title: 'Deixe a listagem impecável',
      publish_fix_store: 'Opcional, mas recomendado — gere o <code>store.json</code> com capa, capturas, tags e descrição longa:',
      publish_fix_store_invalid: 'Seu <code>store.json</code> não é um JSON válido. Regenere ou corrija a sintaxe:',
      ...
      publish_release_fail: 'Nenhuma GitHub Release encontrada. Crie uma tag de versão — o starter já traz o workflow de release.',
      publish_asset_ok: 'Asset da release: %s',
      publish_asset_fail: 'A release mais recente não tem um asset *.ulanziPlugin.zip. Com o starter, envie uma tag depois que o workflow estiver no repo.',
      ...
      publish_store_warn: 'Sem store.json (opcional). Rode o comando store para capa, capturas e tags.',
    ...
    zh: {
      ...
      hero_lead: '一键安装与更新社区插件——或无需 fork 即可发布你自己的插件。',
      hero_download: '免费下载',
      hero_publish: '发布插件',
      ...
      window_install: '安装',
      window_nav_store: '商店',
      window_nav_installed: '已安装',
      window_nav_updates: '更新',
      window_nav_send: '提交插件',
      window_nav_settings: '设置',
      window_star: '在 GitHub 点个星',
      window_badge_updated: '已更新',
      window_badge_new: '新',
      ...
      catalog_all: '全部',
      catalog_all_categories: '全部分类',
      catalog_clear_filters: '清除筛选',
      catalog_count: '%s 个插件',
      ...
      catalog_share: '分享',
      catalog_share_copied: '链接已复制！',
      catalog_open_app: '在应用中打开',
      catalog_source: '源码',
      catalog_report_problem: '报告问题',
      catalog_more_actions: '更多操作',
      catalog_about: '关于',
      catalog_details: '详情',
      catalog_version: '版本',
      ...
      updates_meta_title: '最新动态 — Ulanzi Community Store',
      updates_meta_description: 'Ulanzi Community Store 每周的新插件与更新。分享任意一期 — 链接始终显示同一时间段。',
      updates_eyebrow: '最新动态',
      updates_title: '新插件与更新',
      updates_lead: '所选时间段内进入社区注册表的全部内容。分享链接 — 它始终显示这一时间段。',
      updates_prev: '上一时间段',
      updates_next: '下一时间段',
      updates_range: '%s → %s',
      updates_summary: '%s 个新增 · %s 个更新',
      updates_author_count: '本期 %s 个',
      updates_section_new: '新上架',
      updates_section_updated: '已更新',
      updates_added_on: '%s 上架',
      updates_released_on: '%s 发布',
      updates_to_version: '更新至 v%s',
      updates_more_releases: '本期还有 %s 个版本',
      updates_share: '复制链接',
      updates_share_copied: '链接已复制',
      updates_loading: '正在加载目录…',
      updates_error: '无法加载目录，请稍后再试。',
      updates_empty_title: '这一时间段没有新内容。',
      updates_empty_body: '看看上一周，或浏览完整目录。',
      updates_browse_all: '浏览全部插件',
      ...
      starter_2_title: '搭建或适配',
      starter_2_body: '在空文件夹（新插件）或已有插件仓库（适配模式）中运行下方命令。它会生成完整结构：<code>Makefile</code>、GitHub Actions 发布流水线、可选的 <code>store.json</code>，以及方便 vibe-coding 的 AI skill。',
      starter_store_help: '已有可用插件，只需要封面、截图和标签？',
      starter_docs: '阅读完整文档',
      copy: '复制',
      copied: '已复制！',
      ...
      publish_1_title: '用 starter 搭建或适配',
      publish_1_body: '使用 <a href="#starter-kit">ulanzi-plugin-starter</a> 创建新插件或适配已有仓库——包含 Makefile、发布 workflow 和 <code>store.json</code>。<a href="https://github.com/UlanziTechnology/UlanziDeckPlugin-SDK" target="_blank" rel="noopener noreferrer">Ulanzi 官方 SDK</a> 仍是插件 API 的参考。',
      publish_2_title: '打标签发布 GitHub Release',
      publish_2_body: '推送版本标签（<code>git tag v1.0.0 &amp;&amp; git push origin v1.0.0</code>）。starter 的 workflow 会构建并附加 <code>*.ulanziPlugin.zip</code>。每个新 release 都会成为商店中的更新。',
      publish_3_title: '提交仓库',
      publish_3_body: '在下方粘贴仓库 URL——或在应用的<strong>提交插件</strong>标签页中操作。我们会验证并为你打开提交 issue，剩下的交给机器人。',
      ...
      publish_pr: '在 GitHub 上提交插件',
      publish_copy_json: '复制 JSON',
      publish_hint: '无需 fork：点击按钮会打开预填好的提交 issue。机器人会验证你的仓库并提交 registry Pull Request，合并后插件即自动上线。',
      publish_doc: '需要封面图、截图和标签？在仓库中运行 <code>npx ulanzi-plugin-starter@latest store</code>——或查看 <a href="https://github.com/narlei/ulanzicommunitystore/tree/main/plugin-starter" target="_blank" rel="noopener noreferrer">starter 文档</a>。',
      publish_store_nudge: '可选：生成 <code>store.json</code> 以添加封面、截图和标签：',
      publish_fix_title: '如何修复',
      publish_fix_repo_title: '仓库 URL 无效',
      publish_fix_repo: '请使用公开的 GitHub URL，例如 <code>https://github.com/you/your-plugin</code>（或 <code>you/your-plugin</code>）。',
      publish_fix_release_title: '还没有 release',
      publish_fix_release: '若使用了 starter，推送版本标签即可——workflow 会自动附加 zip：',
      publish_fix_asset_title: 'Release 缺少插件 zip',
      publish_fix_asset: '最新 release 需要 <code>*.ulanziPlugin.zip</code> 资源。使用 starter workflow 时，确保插件文件夹在仓库根目录，然后重新打标签：',
      publish_fix_manifest_title: '插件目录结构问题',
      publish_fix_manifest: '期望在仓库根目录有 <code>com.&lt;you&gt;.&lt;plugin&gt;.ulanziPlugin/manifest.json</code>。在仓库内运行 <strong>init</strong> 可适配布局且不改写代码：',
      publish_fix_store_title: '让商店列表更出色',
      publish_fix_store: '可选但推荐——生成 <code>store.json</code>，用于封面、截图、标签和长描述：',
      publish_fix_store_invalid: '你的 <code>store.json</code> 不是有效 JSON。请重新生成或修复语法：',
      ...
      publish_release_fail: '未找到 GitHub Release。请先打上版本标签——starter 已包含发布 workflow。',
      publish_asset_ok: 'Release 资源文件：%s',
      publish_asset_fail: '最新 release 中没有 *.ulanziPlugin.zip。使用 starter 时，在 workflow 就绪后推送版本标签。',
      ...
      publish_store_warn: '没有 store.json（可选）。运行 store 命令可添加封面、截图和标签。',
