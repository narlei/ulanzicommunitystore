...
(function (global) {
  ...
  var strings = {
    en: {
      ...
      modal_terminal_body: 'Skips the Gatekeeper security prompt entirely — files fetched with <code>curl</code> aren\'t quarantined the way browser downloads are, so the app opens right away.',
      modal_view_script: 'View install.sh source →',
      modal_or: 'or download directly',
      modal_all_releases: 'All releases →',
      modal_note: 'The <code>.dmg</code> triggers a normal macOS "unidentified developer" warning on first launch, since the app isn\'t Apple-notarized (no paid developer account). That\'s expected — see the <a href="https://github.com/narlei/ulanzicommunitystore#-releases" target="_blank" rel="noopener noreferrer">README</a> for how to open it, or just use the Terminal install above instead.',
    ...
    pt: {
      ...
      modal_terminal_body: 'Pula por completo o aviso de segurança do Gatekeeper — arquivos baixados com <code>curl</code> não entram em quarentena como downloads do navegador, então o app abre na hora.',
    ...
    zh: {
      ...
      modal_terminal_body: '完全跳过 Gatekeeper 安全提示——用 <code>curl</code> 获取的文件不会像浏览器下载那样被隔离，应用可立即打开。',
